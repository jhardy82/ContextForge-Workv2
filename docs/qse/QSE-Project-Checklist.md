## QSE – Project Checklist

This checklist specializes the unified template for **QSE (Quantum Sync Engine)** work at the project level.

### 0. Project Header & Personas

- **Project / Workstream**: `QSE Core & Workflows`  
- **Subsystem(s)**: `QSE Engine | QSE CLI | QSE Artifacts (.QSE) | CF_CORE`  
- **Primary Authority**: `QSE` (for phases/evidence) + `CF_CORE` (for orchestration)  
- **Work ID / Epic**: `W-QSE-PHASE6-TESTING-001` / `W-QSE-PHASE7-TESTING-001` (and related)  
- **Current Phase (UTMW)**: `Understand → Think`  

**Personas Involved**

- [x] Researcher (`@researcher`) – analyze QSE workflows, artifacts, logs  
- [x] Implementer (`@implementer`) – QSE engine/CLI/test harness updates  
- [x] QA / Validator (`@qa-reviewer`) – QSE test checklists & evidence validation  
- [x] QSE Orchestrator (`@scientific-method`, `@task-workflow-validation-swarm`) – phase 0–8 orchestration  
- [x] COF / UCL Steward (`@cof-13d-analyst`, `@utmw-trust`) – constitutional alignment  
...
|------|---------------------------------------------------------------|----------------|-------|----------|---------------|
| S-01 | Catalog existing QSE work IDs and YAML checklists             | @researcher    | U     | ☐        | `.QSE/v2/artifacts/*Test.Checklist*.yaml` |
| S-02 | Define canonical QSE phase templates (0–8)                    | @scientific-method | T | ☐        | …             |
| S-03 | Align QSE evidence model with COF/UCL requirements            | @cof-13d-analyst | T   | ☐        | …             |
| S-04 | Implement/adjust QSE CLI commands to emit required evidence   | @implementer   | M     | ☐        | …             |
| S-05 | Validate QSE workflows via QSE CLI + logs + YAML artifacts    | @qa-reviewer   | W     | ☐        | …             |
...
- [ ] `logs/cf_cli.log` (QSE-triggered CF_CLI sessions and orchestration)  
- [ ] `logs/system-operations.log` (system-level workflows touching QSE)  
- [ ] QSE-specific logs (for example, `logs/qse-engine.log` if present)  
- [ ] Gamification / nudge logs when QSE workflows are invoked via gamification flows  
