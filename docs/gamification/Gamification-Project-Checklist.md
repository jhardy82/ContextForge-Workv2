## Gamification – Cross-Project Checklist

This checklist specializes the unified template for **gamification work across the repo** (engine, docs, QSE, and integration).

### 0. Project Header & Personas

- **Project / Workstream**: `ContextForge Gamification`  
- **Subsystem(s)**: `Gamification Engines | Docs | QSE Hooks | TaskMan-v2 Integration`  
- **Primary Authority**: `CF_CORE` / gamification owners  
- **Work ID / Epic**: `Gamification-Global`  
- **Current Phase (UTMW)**: `Understand`  

**Personas Involved**

- [x] Researcher (`@researcher`) – map all gamification touchpoints and behavior  
- [x] Implementer (`@implementer`) – apply changes across engine/docs/integration  
- [x] QA / Validator (`@qa-reviewer`) – verify gamification end‑to‑end paths  
- [x] COF / UCL Steward (`@cof-13d-analyst`) – verify ethical/fair use of gamification  
...
|------|-------------------------------------------------------------|-------------|-------|----------|---------------|
| S-01 | Inventory where gamification is used (engines, UI, QSE, docs) | @researcher  | U     | ☐        | `gamification/`, docs/* |
| S-02 | Define global gamification principles (COF/UCL-aligned)     | @cof-13d-analyst | T  | ☐        | …             |
| S-03 | Ensure all gamification components reference shared principles | @implementer | M   | ☐        | …             |
| S-04 | Validate that gamification is non‑harmful and evidence‑backed | @qa-reviewer | W   | ☐        | logs + tests  |
...
- [ ] `logs/cf_cli.log`  
- [ ] `logs/mcp-api-*.log` (backend)  
- [ ] `logs/mcp-frontend-*.log` (frontend)  
- [ ] `logs/system-*.log` / infra logs  
- [ ] QSE / Gamification logs: `…`  
