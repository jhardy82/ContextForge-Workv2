/**
 * Comprehensive validation of all Task MCP tools
 * Tracks progress using Action List tools
 *
...
console.log("=== Comprehensive Task MCP Tools Validation ===\n");
...
async function validateAllTaskTools() {
  try {
    ...
    try {
      ...
      console.error(`❌ Project ${PROJECT_ID} not found. Please run create-validation-action-lists.mjs first.`);
      throw error;
    }
    ...
    try {
      ...
      if (task.estimated_hours !== 4.0) {
        throw new Error("Estimated hours not updated");
      }
    ...
    try {
      ...
      const verifyResult = await sendRequest("tools/call", {
        name: "task_read",
        arguments: { taskId: testTaskIds[0] }
      });

      if (verifyResult.structuredContent.task.priority !== "high") {
        throw new Error("Bulk update did not apply changes");
      }
    ...
    // Test 9: task_bulk_assign_sprint (requires sprint)
    try {
      console.log("Test 9: task_bulk_assign_sprint");

      // Try to create a sprint first
      let sprintId = null;
      try {
      ...
      if (result.structuredContent.assigned_count !== testTaskIds.length) {
        throw new Error(`Expected ${testTaskIds.length} assignments, got ${result.structuredContent.assigned_count}`);
      }
    ...
    try {
      ...
      const task = await sendRequest("tools/call", {
        name: "task_read",
        arguments: { taskId: testTaskId }
      });

      if (task.structuredContent.task.project_id !== PROJECT_ID) {
    ...
    try {
      ...
      try {
        ...
        if (error.message.includes("at least one") || error.message.includes("Invalid")) {
