...
from taskman_api.core.enums import PhaseStatus, ProjectStatus
...
class ProjectService(
    ...
    async def search(
        ...
        limit: int = 100,
        offset: int = 0,
    ) -> Result[tuple[list[ProjectResponse], int], AppError]:
        ...
        Returns:
            Result containing (projects, total_count) or error
        """
        try:
            repo_status = status.value if status else None

            projects, total = await self.project_repo.search(
                status=repo_status,
                owner=owner,
                limit=limit,
                offset=offset,
            ...
            responses = [
                self.response_class.model_validate(self._deserialize_json_fields(project))
    ...
    async def get_by_phase_status(
        ...
        """Get projects by phase status.

        Args:
            phase_name: Phase name (research, planning)
            phase_status: Phase status to filter by
            limit: Maximum results
            offset: Results to skip

        Returns:
            Result containing projects in specified phase status or error

        Example:
            result = await service.get_by_phase_status("research", PhaseStatus.IN_PROGRESS)
            match result:
                case Ok(projects):
                    print(f"Found {len(projects)} projects in research phase")
        """
        result = await self.project_repo.find_by_phase_status(
    ...
    async def get_with_blocked_phases(
        ...
        """Get projects that have any phase blocked.
        ...
        Returns:
            Result containing projects with blocked phases or error

        Example:
            result = await service.get_with_blocked_phases()
            match result:
                case Ok(projects):
                    print(f"Found {len(projects)} projects with blocked phases")
        """
        result = await self.project_repo.find_with_blocked_phase(limit, offset)
    ...
    async def get_by_current_phase(
        ...
        """Get projects currently in a specific phase (phase is in_progress).
        ...
        Returns:
            Result containing projects in the specified phase or error

        Example:
            result = await service.get_by_current_phase("planning")
            match result:
                case Ok(projects):
                    print(f"Found {len(projects)} projects in planning phase")
        """
        result = await self.project_repo.find_by_current_phase(phase_name, limit, offset)
