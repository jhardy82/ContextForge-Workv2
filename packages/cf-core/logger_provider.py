"""Logger provider infrastructure for ContextForge with correlation tracking.

Implements correlation_id propagation from CLI → Runtime → Plugins per
ContextForge "Logs First" principle. Supports both structlog and stdlib logging.

Authority: P-CF-CLI-ALIGNMENT-02
Constitution: Session correlation rules (CF-133)

HostPolicy: Python >=3.11
"""
...
def get_logger(name: str = "cf_core") -> logging.Logger:
    """Get a stdlib logger by name (legacy API)."""
...
class _JsonFormatter(logging.Formatter):
    """JSON formatter that includes all extra fields in log output.

    Ensures correlation_id and other context fields appear in log records.
    """

    def format(self, record: logging.LogRecord) -> str:
        """Format log record as JSON with all extra fields."""
        log_data = {
        ...
        if hasattr(record, "plugin_scope_id"):
            log_data["plugin_scope_id"] = record.plugin_scope_id
        ...
        for key, value in record.__dict__.items():
            if key not in (
                ...
                "message", "correlation_id", "plugin_id", "plugin_scope_id",
            ):
...
# ==============================================================================
# Runtime Builder Pattern (CF-133)
# ==============================================================================
